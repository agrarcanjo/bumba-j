...
import com.bumbatech.bumbalearning.service.AttemptQueryService;
...
/**
 * REST controller for managing {@link com.bumbatech.bumbalearning.domain.Attempt}.
 */
@RestController
...
public class AttemptResource {
    ...
    public AttemptResource(AttemptService attemptService, AttemptRepository attemptRepository, AttemptQueryService attemptQueryService) {
    ...
    public ResponseEntity<AttemptDTO> createAttempt(@Valid @RequestBody AttemptDTO attemptDTO) throws URISyntaxException {
        ...
        attemptDTO = attemptService.save(attemptDTO);
        return ResponseEntity.created(new URI("/api/attempts/" + attemptDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, attemptDTO.getId().toString()))
            .body(attemptDTO);
    }
    ...
    public ResponseEntity<AttemptDTO> updateAttempt(
        ...
        attemptDTO = attemptService.update(attemptDTO);
    ...
    public ResponseEntity<List<AttemptDTO>> getAllAttempts(
        ...
        LOG.debug("REST request to get Attempts by criteria: {}", criteria);

        Page<AttemptDTO> page = attemptQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /attempts/count} : count all the attempts.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countAttempts(AttemptCriteria criteria) {
        LOG.debug("REST request to count Attempts by criteria: {}", criteria);
        return ResponseEntity.ok().body(attemptQueryService.countByCriteria(criteria));
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<AttemptDTO> getAttempt(@PathVariable("id") Long id) {
