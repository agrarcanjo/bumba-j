  ...
  return (
    <div>
      <h2 id="question-heading" data-cy="QuestionHeading">
        <Translate contentKey="bumbaApp.question.home.title">Questions</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="bumbaApp.question.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/question/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="bumbaApp.question.home.createLabel">Create new Question</Translate>
          </Link>
        </div>
      </h2>
        ...
        {questionList && questionList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="bumbaApp.question.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('type')}>
                  <Translate contentKey="bumbaApp.question.type">Type</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('type')} />
                </th>
                ...
                <th className="hand" onClick={sort('skill')}>
                  <Translate contentKey="bumbaApp.question.skill">Skill</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('skill')} />
                </th>
                ...
                <th className="hand" onClick={sort('assets')}>
                  <Translate contentKey="bumbaApp.question.assets">Assets</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('assets')} />
                </th>
                <th>
                  <Translate contentKey="bumbaApp.question.topic">Topic</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="bumbaApp.question.createdBy">Created By</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {questionList.map((question, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`bumbaApp.QuestionType.${question.type}`} />
                  </td>
                  <td>
                    <Translate contentKey={`bumbaApp.Language.${question.language}`} />
                  </td>
                  <td>
                    <Translate contentKey={`bumbaApp.Skill.${question.skill}`} />
                  </td>
                  <td>
                    <Translate contentKey={`bumbaApp.DifficultyLevel.${question.level}`} />
                  </td>
                  <td>{question.createdAt ? <TextFormat type="date" value={question.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{question.prompt}</td>
                  <td>{question.content}</td>
                  <td>{question.assets}</td>
                  <td>{question.topic ? <Link to={`/topic/${question.topic.id}`}>{question.topic.name}</Link> : ''}</td>
                  <td>{question.createdBy ? question.createdBy.id : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="bumbaApp.question.home.notFound">No Questions found</Translate>
            </div>
          )
        )}
      </div>
