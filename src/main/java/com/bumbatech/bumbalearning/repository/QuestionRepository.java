...
public interface QuestionRepository extends JpaRepository<Question, Long>, JpaSpecificationExecutor<Question> {
    ...
    @Query(
        value = "select question from Question question left join fetch question.topic",
        countQuery = "select count(question) from Question question"
    )
    Page<Question> findAllWithToOneRelationships(Pageable pageable);

    @Query("select question from Question question left join fetch question.topic")
